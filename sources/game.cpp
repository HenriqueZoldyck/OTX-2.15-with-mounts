	...
	const std::string& text, const uint32_t& statementId, bool notify/*= true*/, bool fakeChat/*= false*/)
{
	...
	if(player->getLevel() < 5000 && notify)
	{
		player->sendTextMessage(MSG_STATUS_SMALL, "You need level 5000+ for send Private message.");
		return false;
	}
		...
		if(damage > 0)
		{
			...
			if(useCritAndDodge && attacker && target)
			{
				...
				if((ptarget = target->getPlayer()))
				{
					...
					if((ptarget->getStorage("98798644", valuedod)))
					{
				...
				if(damage > 0 && (pattack = attacker->getPlayer()))
				{
					...
					if((pattack->getStorage("48903", valuecrit)))
					{
						if(!valuecrit.empty() && (std::stoi(valuecrit)) > random_range(0, 1000))
						{
							damage *= 2;
							addAnimatedText(attacker->getPosition(), COLOR_RED, "Critical!");
						}
					}
				}
			}
